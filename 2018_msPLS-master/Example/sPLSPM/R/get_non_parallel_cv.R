get_non_parallel_cv <- function(X,
  ...
  ## RDA objective function (Redundancy analysis (RDA))
  if(nmodes[1]!="B" || nmodes[2]!="B"){
    print("RDA style test")
    #measure time
    stime <- system.time({
      for (l in 1:length(lambdas)){
        ...
        for (nz in 1:length(non_zeros)){
          ...
          for (i in 1:nr_subsets){
            ...
            if(is.null(dim(Y)))  Y<- as.data.frame(as.numeric(Y)) 
            
            Y.train   <- Y[label!=i,]
            Y.test    <- Y[label==i,]

            dim(Y.train);dim(Y.test)
            if(is.null(dim(Y.train)) | is.null(dim(Y.train))){
...
#             Test Variables
            ...
            sub_results[[i]] <- splspm(Data = ndata_sets,
                                       path_matrix = npath_matrix,
                                       blocks = nblocks,
                                       modes = nmodes,
                                       scheme="path",
