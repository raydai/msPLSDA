...
get_cross_validated_penalty_parameters <- function(data_sets,
  ...
  # cross validate datasets if they are connected
        ...
        for (set_connected_checker in 1:length(current_set_path)){
          ...
          if(current_set_path[set_connected_checker]==1  ||
            ...
            if (nr_sets>1 &&
                ...
                #save current dataset in variable
                ...
                if(is.null(dim(matrix_Y)))  {
                  matrix_Y <- data.frame(matrix_Y)
                  colnames(matrix_Y) <- "myGroup"
                  # gens
                  # gens$mvs_names
                }
                ...
                print(set_connected_checker)
                print(specs$modes[c(nr_sets,set_connected_checker)])
                # Split datasets based on the group info.
                if(is.null(dim(matrix_Y))){
                  print("A")
                  shuffled <-  get_split_sets_groups_info(X = matrix_X, Y = matrix_Y,
                                                          nr_subsets = nr_subsets)
                }else{
                  print("B")
