# X = matrix_X
# Y = data_sets[,blocks[[set_connected_checker]]]
# Y = data_sets[,blocks[[3]]]
# nr_subsets = nr_subsets

get_split_sets_groups_info <- function(X, Y, nr_subsets){
  ...
  Y.sampled <- as.data.frame(Y[splitting_dimensions,])
