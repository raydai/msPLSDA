...
run1 <- readRDS(file = "Rcode/RealDataAnalysis/Final_LOS_results_run1.RDS")
run2 <- readRDS(file = "Rcode/RealDataAnalysis/Final_LOS_results_run2.RDS")
feature_slection1 <- run1$model$outer_model %>% 
  filter(weight!=0) %>% 
  transform(name=as.character(name)) %>% 
  filter(name!="BMD")
feature_slection2 <- run2$model$outer_model %>% 
  filter(weight!=0) %>% 
  transform(name=as.character(name)) %>% 
  filter(name!="BMD")
feature_slection <- c(feature_slection1$name,feature_slection2$name)
# View(head(dfCompile))
dfCompile <- dfCompile %>% select(-feature_slection)
...
# cl <- makeCluster(50)
cl <- parallel::makeCluster(detectCores()-12)
doParallel::registerDoParallel(cl)
...
# myRDS <- list.files(path = "Rcode/RealDataAnalysis/FeatureNumberSearch/run3",pattern = ".RDS")
...
# foreach(iter=continu.run,.packages = c("tester","turner","mvtnorm","amap","nnet")) %dopar% {
  ...
  tmp.filename <- paste("Rcode/RealDataAnalysis/FeatureNumberSearch/run3/Out_file_",iter,".RDS",sep = "")
